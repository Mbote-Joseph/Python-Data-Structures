...
def menu():
    ...
    print("5. Search units from the student: ")
...
def update_unit(units, scores):
    ...
    option = int(input("Enter the index of the unit you want to update: "))
    name = input(f"Enter the correct name for {units[option - 1]}: ")
    units[option-1] = name
...
def search(units):
    phrase = input("Enter the phrase you want to search: ")
    try:
        x = units.index(phrase)
        print(units[int(x)])
    except units.index(phrase) == -1:
        print("That unit is not done by this student")
