...
def update_unit(units, scores):
    ...
    name = int(input(f"Enter the correct name for {units[unit - 1]}: "))
    units[option-1] = name
...
def update_score(units, scores):
    ...
    score = int(input(f"Enter the correct score for {units[unit - 1]}: "))
    scores[option-1] = score
    
    return scores

def update(units, score):
    print("1. Unit name")
    print("2. Score")
    option = int(input("You want to update Unit name or score ? (1, 2):"))
    match option:
        case 1:
            update_unit(units, score)
        case 2:
            update_score(units, score)
        case 0:
